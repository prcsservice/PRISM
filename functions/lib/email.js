...
exports.sendMentorActionEmail = sendMentorActionEmail;
exports.sendStressTrendEmail = sendStressTrendEmail;
...
async function sendEmail(params) {
    try {
        ...
        firebase_functions_1.logger.info(`✉️ Email sent: "${params.subject}" → ${params.to}`);
    }
    catch (error) {
        firebase_functions_1.logger.error(`❌ Email failed: "${params.subject}" → ${params.to}`, error);
    }
}
// ===== Base HTML Template =====
 ...
 */
async function sendMentorActionEmail(data) {
    const html = generateEmailHTML({
 ...
 */
async function sendStressTrendEmail(data) {
    const html = generateEmailHTML({
        recipientName: data.teacherName,
